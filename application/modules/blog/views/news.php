	    ...
	    <?php if(isset($news) && $news): ?>
	    	<?php foreach($news as $entry): ?>
				...
				<header style="margin-bottom: 10px;">
					<h2><?php echo anchor('news/'.url_title($entry->title), $entry->title, 'class="none" style="color: #A16E36;"'); ?></h2>
				...
				<div class="details">
					<ul>
						<li>
							...
							Posted by <?php echo anchor(uri_string().'#'.url_title($entry->name), $entry->name, 'class="none"'); ?>
						</li>
						<li>
							<span class="icon color icon42"></span>
							<?php 
								if($entry->count_comment > 1){
									echo anchor('news/'.url_title($entry->title), $entry->count_comment.' Comments', 'class="none"'); 
								}elseif($entry->count_comment == 1){
									echo anchor('news/'.url_title($entry->title), '1 Comment', 'class="none"');
								}else{
									echo anchor('news/'.url_title($entry->title), 'No Comments', 'class="none"');
								}					
							?>
						</li>						
					</ul>
