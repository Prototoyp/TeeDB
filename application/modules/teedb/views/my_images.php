		...
		<div id="info">		
			<?php if(isset($delete) && isset($delete_id)): ?>
				<p class="info border"><span class="icon color icon112"></span>
					Are you really sure you want to remove the <?php echo singular($type).' '.$delete; ?>?
				</p>
			<?php endif; ?>
			<?php 
				...
				<?php foreach($uploads as $entry): ?>
					...
					<li style="height: 245px">
						<?php echo form_open(null, null, array('id' => $entry->id)); ?>
							<img src="<?php echo base_url((($type == 'demos' || $type == 'maps')? 'assets/images/nopic_'.singular($type).'.png' : 'uploads/'.$type.'/previews/'.$entry->name.'.png')); ?>" alt="Preview of <?php echo $type.' '.$entry->name; ?>" />
							<p>
							...
							<?php else: ?>
								<?php echo form_submit('delete', 'Delete '.singular($type), 'style="font-size:10px" class="leight"'); ?>
							<?php endif; ?>
